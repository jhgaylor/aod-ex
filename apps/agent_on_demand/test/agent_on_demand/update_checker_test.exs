defmodule AgentOnDemand.UpdateCheckerTest do
  ...
  # Helpers to start an UpdateChecker with a deterministic release fetcher.
  # Using dependency injection via :release_fetcher avoids needing Mimic to
  # cross process boundaries into the GenServer.

  defp start_checker(fetcher) do
    start_supervised!({UpdateChecker, name: nil, release_fetcher: fetcher})
  end

  describe "get_status/1" do
    test "returns initial state after first check completes" do
      checker = start_checker(fn -> {:ok, %{"tag_name" => "v0.2.17", "assets" => []}} end)
      Process.sleep(50)
  ...
  describe "check_now/1 and PubSub broadcasts" do
    test "broadcasts has_update: true when newer version found" do
      ...
      start_checker(fn -> {:ok, %{"tag_name" => "v99.0.0", "assets" => []}} end)
    ...
    test "broadcasts has_update: false when same or older version" do
      Phoenix.PubSub.subscribe(AgentOnDemand.PubSub, "update_checker")
      start_checker(fn -> {:ok, %{"tag_name" => "v0.0.1", "assets" => []}} end)
    ...
    test "broadcasts checking: true before HTTP call, then false after" do
      Phoenix.PubSub.subscribe(AgentOnDemand.PubSub, "update_checker")
      start_checker(fn -> {:ok, %{"tag_name" => "v0.2.17", "assets" => []}} end)
    ...
    test "handles GitHub API error gracefully without crashing" do
      Phoenix.PubSub.subscribe(AgentOnDemand.PubSub, "update_checker")
      checker = start_checker(fn -> {:error, :timeout} end)
